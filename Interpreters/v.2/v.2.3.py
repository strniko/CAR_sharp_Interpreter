...
rotation = 1
result = ""
single_step = False  # Variable to store the state of the Single Step toggle button
...
def open_gui():
    ...
    if gui_visible:
        ...
        for i in range(max(0, y - 5), min(len(grid), y + 6)):
            for j in range(max(0, x - 5), min(len(grid[0]), x + 6)):
                cell_value = grid[i][j]

                # Highlight the cell at the current position
                bg_color = "#0AF" if (i, j) == (y, x) else "white"

                cell_label = tk.Label(grid_frame, text=str(cell_value), width=5, height=2, borderwidth=1,
                                      relief="solid", bg=bg_color)
                cell_label.grid(row=i - max(0, y - 5), column=j - max(0, x - 5))
...
def update_gui():
    for widget in debug_gui.winfo_children():
        widget.destroy()
    x, y = position
    grid_frame = tk.Frame(debug_gui)
    grid_frame.pack(padx=10, pady=10)
    ...
    for i in range(max(0, y - 5), min(len(grid), y + 6)):
        for j in range(max(0, x - 5), min(len(grid[0]), x + 6)):
            ...
            cell_label = tk.Label(grid_frame, text=str(cell_value), width=5, height=2, borderwidth=1, relief="solid",
                                  bg=bg_color)
            cell_label.grid(row=i - max(0, y - 5), column=j - max(0, x - 5))
